...
old_main_start = """  // ─── AUTHENTICATED COMMAND CENTER RENDER ─────────────────────────────────────
  return (
...
new_main_start = """  // ─── AUTHENTICATED COMMAND CENTER RENDER ─────────────────────────────────────
  return (
    <div className={`min-h-screen ${theme.pageBg} text-gray-800 antialiased flex flex-col md:flex-row max-w-[1600px] mx-auto`}>

      {/* ── SIDEBAR (DESIGNER TOOLKIT) ── */}
      <aside className="w-full md:w-72 shrink-0 border-r border-black/5 flex flex-col h-screen sticky top-0 bg-white shadow-xl shadow-black/5 z-50">
      ...
      <main className="flex-1 min-w-0 flex flex-col h-screen overflow-y-auto custom-scrollbar">
        ...
        <header className={`sticky top-0 z-40 ${theme.headerBg} backdrop-blur-md border-b border-black/5 shadow-sm`}>
          <div className="px-4 sm:px-8 py-3 flex items-center justify-between gap-4 flex-wrap">"""
...
# We need to add the Dashboard Overview Metrics right after the <header> tag ends.
# Let's find where the <header> ends.
header_end = """        </header>

        {/* ── MAIN AREA ── */}
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6 sm:space-y-8 w-full">"""

dashboard_content = """        </header>

        {/* ── MAIN AREA ── */}
        <div className="max-w-6xl mx-auto px-4 sm:px-8 py-6 sm:py-8 space-y-8 w-full">
...
content = content.replace(header_end, dashboard_content)

# Fix the end tags: we need to close </main> before the final </div> that wraps the AnimatePresence modals
# Let's find the closing of the `min-h-screen` container.
...
eof_replacement_old = """        </div>

      </div>

      {/* ── DUE TODAY ALERTS ── */}"""

eof_replacement_new = """        </div>
      </main>

      {/* ── DUE TODAY ALERTS ── */}"""

content = content.replace(eof_replacement_old, eof_replacement_new)

with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)

print("Patching Dashboard V4 successful!")
